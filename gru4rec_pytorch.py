...
class GRUEmbedding(nn.Module):
    def __init__(self, dim_in, dim_out):
        ...
        self.Wx0 = nn.Embedding(dim_in, dim_out * 3, sparse=False)
...
class GRU4RecModel(nn.Module):
    def __init__(
        ...
        if constrained_embedding:
            ...
            self.E = nn.Embedding(n_items, embedding, sparse=False)
            n_input = embedding
        else:
        ...
        self.Wy = nn.Embedding(n_items, layers[-1], sparse=False)
        self.By = nn.Embedding(n_items, 1, sparse=False)
        self.reset_parameters()
...
class GRU4Rec:
    ...
    def fit(
        ...
        # assign distributed data parallel model
        ...
        for epoch in range(self.n_epochs):
            ...
            if not dist.is_initialized() or dist.get_rank() == 0:
                pbar = tqdm(desc="Epoch{} ".format(epoch + 1), unit="step", leave=True)
            else:
                pbar = None
            t_step = time.time()
            ...
            for in_idx, out_idx in self.data_iterator(
                ...
                # time per 10 steps logging
                if pbar is not None:
                    pbar.update(1)
                    if train_step % log_interval == 0 and train_step > 0:
                        t2 = time.time()
                        dt = t2 - t_step
                        step_ms = dt / log_interval * 1000

                        pbar.set_postfix(
                            {
                                "perf": f"{step_ms:.2f} ms/step",
                            }
                        )
    ...
    @classmethod
    def loadmodel(cls, path, device="musa:0"):
