...
def init_distributed(args):
    ...
    if args.world_size > 1:
        if torch.cuda.is_available():
            ...
            torch.musa.set_device(args.local_rank)
...
#    type=str,
#    default="musa:0",
#    help="Device used for computations (default: musa:0).",
# )
parser.add_argument(
    "-ik",
    "--item_key",
...
if args.load_model:
    ...
    gru = GRU4Rec.loadmodel(args.path, device=device)
else:
    ...
    if is_main_process(args):
        print('Creating GRU4Rec model on device "{}"'.format(device))
    ...
    if args.save_model is not None and is_main_process(args):
        print("Saving trained model to: {}".format(args.save_model))
        gru.savemodel(args.save_model)

if args.test is not None and is_main_process(args):
